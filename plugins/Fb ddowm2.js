...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (!args[0]) throw `Use example ${usedPrefix}${command} https://fb.watch/aFRcy/`
    //const { result } = await facebookdlv2(args[0])
    if (!args[1]) return conn.sendButton(m.chat, `* ғᴀᴄᴇʙᴏᴏᴋ*`, null, null, [['sᴅ', `.fb ${args[0]} sd`],['ʜᴅ', `.fb ${args[0]} hd`]],m)
